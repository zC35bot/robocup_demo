...
- `distortion_model: 0` 即 `kNone`（图像已矫正）。
...
`Init` 时还会 `LogYAML(node, "vision_local.yaml")` 把当时的完整配置一起存档（`vision_node.cpp:203`），保证数据集可复现。存盘频率由 `save_fps` 经 `save_every_n_frame_` 控制（[3.1](./3.1-节点与主流水线.md)）。
