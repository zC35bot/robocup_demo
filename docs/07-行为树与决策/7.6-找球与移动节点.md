...
- **场地移动**：`GoToReadyPosition`/`GoToFreekickPosition` 按角色+`myStrikerIDRank` 算专属站位，`GoToGoalBlockingPosition` 门前封堵，`GoBackInField` 把走出界的自己拉回场内，`Assist` 算助攻接应位，底层都走 `moveToPoseOnField`。
