...
> 💡 **安全加固：非视觉踢决策必清 `tmImInVisualKick`。** `StrikerDecide` 在 `if-else` 链末尾补了一句 `if (newDecision != "auto_visual_kick") tmImInVisualKick = false;`（`brain_tree.cpp:918`）。之前这个标志只在命中 auto_visual_kick 时置真、没有对称的清零点，一旦这帧决策变成 chase/adjust/kick 等，`tmImInVisualKick` 会**残留(stale)为真**——它是要通过队友通信广播出去的"我正在视觉踢"状态，残留会让队友误判我仍在踢球而错误让位。现在只要本帧不是 auto_visual_kick 就立即复位，保证该标志与实际决策严格同步。此外，整段决策读写 `tmImLead`/`tmMyCostRank`/`tmImInVisualKick` 等**跨线程共享字段**时已统一用 `brainMutex` 加锁（`brain_tree.cpp:870`），避免与通信线程并发读写产生竞态。
